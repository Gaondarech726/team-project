const API_KEY = 'uGKlDKANEChywwqXRrPXgVln1mL2wAnx';
...
class TicketsApi {
  ...
  async getRandom(page = 0) {
    try {
      const searchRequest = `${BASE_URL}events.json?&apikey=${API_KEY}&page=${page}&size=200&includeTBA=no&includeTBD=no&sort=random`;
      const responce = await fetch(searchRequest);
      const result = await responce.json();
      return result;
    } catch (err) {
